...
for i in range(len(nums)):
    maximum = 0
    if nums[i][cur_index] > maximum:
        maximum = nums[i][cur_index]
    if len(nums[i])> cur_index+1 and nums[i][cur_index+1]>maximum:
        maximum = nums[i][cur_index+1]
        cur_index+=1
    choice.append(maximum)
    total+=maximum
    print(total,maximum)
print(choice)
...
def recSumAtRow(rowData, rowNum):
    ...
    for i in range(len(rowData[rowNum])):
        ...
        p = max([rowData[rowNum+1][i],rowData[rowNum+1][i+1]])
        print(p)
        rowData[rowNum][i] += p 
    # base case
